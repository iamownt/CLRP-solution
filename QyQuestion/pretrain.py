...
from components.optimizer import get_optimizer_robertaMLM, get_scheduler, get_optimizer_bertMLM
import torch
import os
import sys
# import wandb
...
def main():
    ...
    optimizer = get_optimizer_bertMLM(model,config)
    ...
    trainer.train()
    try:
        trainer.save_model("/hhd/wt106/QyQuestion/Qymodels/trainer/ro")
    except:
        print("Wrong save!")
    ...
    for key in list(dict_.keys()):
        dict_[key.replace('bert.', 'base.')] = dict_.pop(key)
    local_rank = torch.distributed.get_rank()  # 防止多个进程互相抢占
    if (local_rank == 0):
        torch.save(dict_, f'./Qymodels/roberta_large_pretrain.pt')
