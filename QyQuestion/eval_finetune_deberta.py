...
def run_fold_eval(fold, config, train_data, tokenizer, t_bar, save_fold_prediction):
    ...
    model.load_state_dict(torch.load(config['save_path'] + f'deberta_large_{fold}.pt', accelerator.device), strict=True)
...
def generate_test_prediction(config, output_dir):
    ...
    total_predictions = []
    config['n_folds'] = 1  #just train 1 fold
    for i in range(config['n_folds']):
...
if __name__ =="__main__":
    ...
    if not infer:
        ...
        # np.save(os.path.join(output_dir, "last_all_prob_de5_long.npy"), last_all_prob)
    else:
        output_dir = "/hhd/wt106/QyQuestion/Qypredictions/"
